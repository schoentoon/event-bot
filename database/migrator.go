...
func migrate(db *sql.DB, insertVersion string, migration *Migration) error {
	...
	log.Println(fmt.Sprintf("migrator: applying migration named '%s'...", migration.Name))
	...
	log.Println(fmt.Sprintf("migrator: applied migration named '%s'", migration.Name))

	return err
}

func migrateNoTx(db *sql.DB, insertVersion string, migration *MigrationNoTx) error {
	log.Println(fmt.Sprintf("migrator: applying no tx migration named '%s'...", migration.Name))
	...
	log.Println(fmt.Sprintf("migrator: applied no tx migration named '%s'...", migration.Name))
